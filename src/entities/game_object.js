...
import { COULOMB_CONSTANT } from "../common/util/util.js";
...
class GameObject {
	...
	updateXPos(deltaT, pixelScale) {
		if (
			this.path.bounds.bottomLeft.x > 0 &&
			this.path.bounds.bottomRight.x < this.game.width
		) {
	...
	updateYPos(deltaT, pixelScale) {
		if (
			this.path.position.y + this.path.bounds.height / 2 <
				this.game.groundYPos ||
			this.velocity.y < 0
		) {
	...
	assignForces() {
		...
		this.conditionalForces = {
			frictionKinetic: new Force(
				gravitationalAcc * this.mass * this.fricCoeff,
				0
			),
			frictionStatic: new Force(
				gravitationalAcc * this.mass * groundFriction.static,
				0
			),
	...
	checkIfOnTopOfObject(gameObject) {
		if (
			...
			((this.path.bounds.bottomLeft.x >= gameObject.path.bounds.topLeft.x &&
				this.path.bounds.bottomLeft.x <= gameObject.path.bounds.topRight.x) ||
				(this.path.bounds.bottomRight.x >= gameObject.path.bounds.topLeft.x &&
					this.path.bounds.bottomRight.x <=
						gameObject.path.bounds.topRight.x)) &&
			//check if vertical space between objects is small
			Math.abs(
				this.path.bounds.bottomRight.y - gameObject.path.bounds.topLeft.y
			) < 0.05
		) {
			return true;
		} else {
	...
	calculateCoulombForce(gameObject) {
		...
		const forceTot =
			(COULOMB_CONSTANT * this.charge * gameObject.charge) / rSquared;
		let forceX;
		let forceY;
	...
	distanceTo(gameObject) {
		return Math.sqrt(
			(gameObject.path.position.x - this.path.position.x) ** 2 +
	...
	checkForCollisions() {
		...
		Object.values(this.game.gameObjects).forEach((gameObject) => {
			if (
				!(this.distanceTo(gameObject) > 1.1 * this.length) &&
				!(this.distanceTo(gameObject) > 1.1 * this.width)
			) {
	...
	correctCollisionPosition(collisionSubject) {
		...
		if (intersection.height <= intersection.width) {
			...
			if (
				(this.path.bounds.bottomLeft.y >
					collisionSubject.path.bounds.topLeft.y ||
					this.path.bounds.bottomRight.y >
						collisionSubject.path.bounds.topLeft.y) &&
			...
			if (
				(this.path.bounds.bottomLeft.x <
					collisionSubject.path.bounds.topRight.x ||
					this.path.bounds.topLeft.x <
						collisionSubject.path.bounds.topRight.x) &&
				(this.path.bounds.bottomRight.x >
					collisionSubject.path.bounds.topRight.x ||
	...
	setCollisionVelocities(gameObject, axis) {
		...
		if ((gameObject.touchingGround || gameObject.grounded) && axis == "y") {
			...
			if (this.velocity.x < -0.01) {
				this.velocity.x +=
					(this.deltaT * this.conditionalForces.frictionKinetic.x) / this.mass;
		...
		if (Math.abs(initialVelocity[axis]) > 0.02) {
			...
			gameObject.velocity[axis] =
				...
				((2 * initialVelocity[axis] * this.mass) / totalMass -
					((this.mass - gameObject.mass) / totalMass) *
						gameObject.velocity[axis]);
		}
	}
}
